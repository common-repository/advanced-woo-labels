...
if ( ! class_exists( 'AWL_Admin' ) ) :
    ...
    class AWL_Admin {
        ...
        public function add_admin_page() {
            ...
            add_menu_page( esc_html__( 'Adv. Woo Labels', 'advanced-woo-labels' ), esc_html__( 'Adv. Woo Labels', 'advanced-woo-labels' ), 'manage_options', $slug, false, 'dashicons-tag', 58);

            add_submenu_page( $slug, __( 'All Labels', 'advanced-woo-labels' ), __('All Labels', 'advanced-woo-labels'), 'manage_options', $slug );
            add_submenu_page( $slug, __( 'Add New', 'advanced-woo-labels' ), __('Add New', 'advanced-woo-labels'), 'manage_options', 'post-new.php?post_type=awl-labels' );
        ...
        public function save_meta_boxes( $post_id, $post ) {
            ...
            update_post_meta( $post_id, '_awl_label', $label );
        ...
        public function metabox_show_rules() {
            ...
            $label = get_post_meta( $post->ID, '_awl_label', true );
        ...
        public function admin_enqueue_scripts() {
            ...
            if ( ( isset( $_GET['page'] ) && $_GET['page'] == 'awl-options' ) || ( $screen && $screen->post_type && $screen->post_type === 'awl-labels' ) ) {
                ...
                wp_enqueue_script( 'awl-admin-scripts', AWL_URL . '/assets/js/admin.js', array('jquery', 'wp-color-picker' ), AWL_VERSION );
